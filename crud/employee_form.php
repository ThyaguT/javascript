...
$name=$_POST['name'];
    ...
    $filename=$_FILES["photo"]["name"]; 
     ...
     if($uploadOk==0){ 
        ...
        $sql =mysqli_query($conn,"INSERT INTO emp_details(name,email,phone,password,photo) VALUES('$name','$email','$phone','$hash','$image')");
        $data=mysqli_insert_id($conn);
        $sql1=mysqli_query($conn,"INSERT INTO login(loginid,email,password) VALUES ('$data','$email','$hash')");
if($sql1)
{
    echo'<script> alert("registered successfully");
    window.location.href="emplo_log.php";</script>';
}
else{
